...
@app.route(f"/{API_PREFIX}/analyze-all", methods=["POST"])
def analyze_all_api():
    """
    Análisis vía API usando todos los motores (como 'All' en la web).
    Devuelve los resultados completos (espera a que termine).
    """
    import time
    ...
    observables = extract_observables(fanged_text)

    if not observables:
        return jsonify({"error": "No valid observables found."}), 400

    # ⚠️ Si GUI_ENABLED_ENGINES está vacío, usar motores por defecto
    selected_engines = GUI_ENABLED_ENGINES or [
        "ipquery",
        "spur",
        "urlscan",
        "reverse_dns",
    ...
    # Crear hash de caché
    combined_data = f"{text}|{','.join(selected_engines)}"
    cache_key = f"analyze-all-{hashlib.sha256(combined_data.encode('utf-8')).hexdigest()}"
    ...
    threading.Thread(
        target=perform_analysis,
        args=(app, observables, selected_engines, analysis_id),
    ).start()

    # Esperar hasta que termine (máx 30 segundos)
    max_wait = 30
    elapsed = 0
    while check_analysis_in_progress(analysis_id) and elapsed < max_wait:
        time.sleep(1)
        elapsed += 1

    if check_analysis_in_progress(analysis_id):
        return jsonify({"error": "Timeout: analysis still in progress"}), 202
    ...
    if result:
        ...
        cache.set(cache_key, response, timeout=secrets.api_cache_timeout)
        return jsonify(response), 200

    return jsonify({"error": "Analysis failed or not found"}), 500
